    ...
    >
      <nav className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-8 md:px-12 lg:px-16">
        <a href="/" className="flex items-center">
          <img src="/images/logo.png" alt="Contour Beauty" className="h-9 w-auto"
        ...
        <button className="flex lg:hidden flex-col items-center justify-center gap-1.5 w-10 h-10"
          onClick={() => setIsMobileMenuOpen((p) => !p)} aria-label="Menü">
          <motion.span className="block h-px w-6 origin-center"
            style={{ backgroundColor: scrolled ? "var(--cb-charcoal)" : "white" }}
            animate={isMobileMenuOpen ? { rotate: 45, y: 4 } : { rotate: 0, y: 0 }} transition={{ duration: 0.25 }} />
          <motion.span className="block h-px w-6"
            style={{ backgroundColor: scrolled ? "var(--cb-charcoal)" : "white" }}
            animate={isMobileMenuOpen ? { opacity: 0 } : { opacity: 1 }} transition={{ duration: 0.15 }} />
          <motion.span className="block h-px w-6 origin-center"
            style={{ backgroundColor: scrolled ? "var(--cb-charcoal)" : "white" }}
            animate={isMobileMenuOpen ? { rotate: -45, y: -4 } : { rotate: 0, y: 0 }} transition={{ duration: 0.25 }} />
        </button>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            {navLinks.map((link, i) => (
              <motion.a key={link.href} href={link.href} className="cb-nav-link text-sm"
                initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }}
                onClick={() => setIsMobileMenuOpen(false)}>{link.label}</motion.a>
            ))}
