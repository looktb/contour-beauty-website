...
export function Navbar9() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[999] transition-all duration-500"
      style={{ backgroundColor: scrolled ? "rgba(250,248,244,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", borderBottom: scrolled ? "1px solid rgba(28,25,23,0.08)" : "none" }}>
      <nav className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-8 md:px-12 lg:px-16">
        ...
        <div className="hidden lg:flex items-center gap-10">
          {links.map((l) => <a key={l.href} href={l.href} className="cb-nav-link"
            style={{ color: scrolled ? "var(--cb-charcoal)" : "rgba(250,248,244,0.9)" }}>{l.label}</a>)}
        </div>
        <a href="/kontakt" className="hidden lg:flex cb-btn-primary"
